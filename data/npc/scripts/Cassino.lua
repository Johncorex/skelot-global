local config = {
    ...
    position = {
        player = Position(32354, 32226, 7),  -- Posição que o player deve estar
        money = Position(32353, 32225, 7),   -- Posição do dinheiro
        dice = Position(32352, 32225, 7)     -- Posição de onde estará o dado
    },
    effects = {
...
local function creatureSayCallback(cid, type, msg)
    ...
    if not npcHandler:isFocused(cid) or player:getStorageValue(7239) >= os.stime() then
        return false
    end
    
    player:setStorageValue(7239, os.stime() + 2)
...
function onThink()
    ...
    if player then
        if not playerIsInPosition(player) then
            npcHandler:say("Farewell, " .. player:getName(), player.uid)
...
function removeMoney()
    ...
    if money < config.min then
        npcHandler:say("You need at least " .. config.min .. " gps to use the cassino.", player.uid)
        return false
    elseif money > config.max then
        npcHandler:say("You can only use " .. config.max .. " gps coins in the casino", player.uid)
        return false
    end
